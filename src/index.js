 ...
 * @Last Modified by: yepanmeng
 * @Last Modified time: 2018-09-13 10:02:32
 */
...
CaptureColor.prototype.transformToImg = function(node) {
    let _clientRect = node.getBoundingClientRect()
    html2canvas(node, {
        useCORS: true
    }).then(data => {
        let _data = data
        _data.id = this.uuid
        _data.style.cssText = `position: absolute;top:${_clientRect.top}px;left:${_clientRect.left}px;width:${_clientRect.width}px;height:${_clientRect.height}px;z-index:11111111 !important;`
        ...
        _data.onmousemove = (coordinate) => {
            ...
            _canvas_all.style.top = (coordinate.y < _clientRect.top + _clientRect.height - 150 ? coordinate.y + 15 : coordinate.y - 150 - 15) + 'px'
            _canvas_all.style.left = (coordinate.x < _clientRect.left + _clientRect.width - 150 ? coordinate.x + 15 : coordinate.x - 100 - 15) + 'px'
            this.color = _color
        }
